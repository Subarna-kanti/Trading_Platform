...
async def create_order(order: schemas.OrderCreate, db: Session = Depends(get_db)):
    ...
    if order.type == models.OrderType.buy:
        if not order.price:
            raise HTTPException(status_code=400, detail="Price required for limit buy orders")
        total_cost = order.price * order.quantity
        if wallet.balance < total_cost:
            raise HTTPException(status_code=400, detail="Insufficient balance to place buy order")
        wallet.balance -= total_cost  # reserve funds

    # Sellers don’t reserve balance here (simplified version)
    # Later we could introduce asset reservations
    ...
    # Broadcast updated wallet balance (only for buyers who had balance deducted)
    if order.type == models.OrderType.buy:
...
def delete_order(order_id: int, db: Session = Depends(get_db)):
    ...
    # If deleting a pending BUY order, refund reserved balance
    if db_order.type == models.OrderType.buy and db_order.status == models.StatusType.pending:
        wallet = db.query(models.Wallet).filter(models.Wallet.user_id == db_order.user_id).first()
        if wallet:
